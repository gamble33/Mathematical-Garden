...
Benefit: Every possible field (piece of information/data) will be listed and the ones which aren't necessary would be eliminated.
...
FUNCTION SearchInRow(Row : INTEGER, StartColumn : INTEGER) RETURNS INTEGER
    ...
    WHILE (i >= 0 AND i <= 1280) AND (Flag = FALSE)
...
## c
```fortran
FUNCTION GetCentreCol(Row : INTEGER) RETURNS INTEGER
    DECLARE Start : INTEGER
    DECLARE End : INTEGER

    Start <- SearchInRow(Row, 1)
    End <- SearchInRow(Row, 1280)
    ...
    RETURN (Start + End) DIV 2
ENDFUNCTION
```

> [!offtopic]
> ofc use `DIV` for integer division to avoid rounding errors and what not. right?
